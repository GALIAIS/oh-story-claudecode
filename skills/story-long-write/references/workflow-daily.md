...
1. **读取细纲**：加载本次要写的 2-3 章细纲。新版细纲优先读取「内容概括（起因/发展/转折/高潮/结尾）」「情节安排（主线/辅线/事件线/感情线/逻辑线）」「人物关系和出场顺序」「情节细化」「结尾设定和钩子」；旧版细纲缺这些字段不阻塞，回退到核心事件、情节点序列、目标情绪、章首/章尾钩子、字数目标。
2. **逐章执行**（以下每步在每章循环内执行）：
   - 读细纲 → 按需加载角色设定
   ...
   - 写正文 → **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选，< 目标90%则对照情节点字数预算定位欠账密点、一次性重写到配额，不挤牙膏反复回炉）** → 检查钩子/爽点 → **正文元信息扫描** → 禁用词扫描
